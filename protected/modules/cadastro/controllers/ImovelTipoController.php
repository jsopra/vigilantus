...
class ImovelTipoController extends Controller
{
	...
	{
		$model=new ImovelTipo('search');
		$model->unsetAttributes();  // limpa quaisquer valores padrão
		if(isset($_GET['ImovelTipo']))
			$model->attributes=$_GET['ImovelTipo'];
	...
	{
	   if (Yii::app()->request->isPostRequest && isset($_POST['ImovelTipo'])) {

		   $model = $id ? $this->loadModel($id) : new ImovelTipo;

		   $model->attributes = $_POST['ImovelTipo'];
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='imovel-tipo-form')
		{
