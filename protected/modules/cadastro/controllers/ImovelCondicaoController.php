...
class ImovelCondicaoController extends Controller
{
	/**
	...
	{
		$model=new ImovelCondicao('search');
		$model->unsetAttributes();  // limpa quaisquer valores padrão
		if(isset($_GET['ImovelCondicao']))
			$model->attributes=$_GET['ImovelCondicao'];
	...
	{
	   if (Yii::app()->request->isPostRequest && isset($_POST['ImovelCondicao'])) {

		   $model = $id ? $this->loadModel($id) : new ImovelCondicao;

		   $model->attributes = $_POST['ImovelCondicao'];
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='imovel-condicao-form')
		{
