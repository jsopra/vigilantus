...
/**
 * Baseado na classe MbMenu de Mark van den Broek (mark@heyhoo.nl) Copyright &copy; 2010 HeyHoo
 *
 */
...
class PMenu extends CMenu
{
	...
	/**
	 * Nome do arquivo CSS chamado
	 * 
	 * false = nenhum
	 * null = arquivo padrão (webol.extensions.fidelize.menu.css.menu)
	 * string = carrega o arquivo informado
	 * 
	 * @var string|boolean 
	 */
	public $cssFile;
	
	public $containerId = 'nav-container';
	public $barId = 'nav-bar';
	
	/**
	 * Se ativado, só renderiza itens do menu que tenham um link ou que tenham
	 * pelo menos um filho visível.
	 * 
	 * @var boolean
	 */
	public $onlyParentsWithChildrenOrUrl = true;
	...
	{
		$js = '';
		$js .= '  $("#' . $this->getId() . ' li").hover(' . $this->nljs;
	...
	public function registerClientScripts()
	{
		...
		$cs->registerScript('fmenu_' . $this->getId(), $js, CClientScript::POS_READY);
	}
	...
	public function registerCssFile($url = null)
	{
		if ($url !== false) {
			...
			if ($url == null) {
				...
				if ($this->baseUrl === '')
					throw new CException(Yii::t('PMenu', 'baseUrl must be set. This is done automatically by calling publishAssets()'));
				
				$url = $this->baseUrl . '/css/menu.css';
			}
			
			Yii::app()->getClientScript()->registerCssFile($url, 'screen');
		}
	}
		...
		foreach ($items as $i => $item) {
			...
			if (isset($item['items'])) {
				...
				if (empty($items[$i]['items']) && $this->hideEmptyItems) {
					...
					if ($this->onlyParentsWithChildrenOrUrl && !isset($item['url'])) {
	...
	public function run()
	{
		...
		echo CHtml::openTag('div', array('id' => $this->containerId)) . "\n";
			...
			echo CHtml::openTag('div', array('id' => $this->barId)) . "\n";
