...
<div class="bairro-form">
    ...
    <?php $form = ActiveForm::begin(); ?>
        ...
        <div class="form-group form-actions">
            ...
            echo Html::a(
                'Cancelar',
                array('/armadilha/index'),
                array('class'=>'link','rel'=>'tooltip', 'data-role' => 'cancel','data-title'=>'Ir à lista de armadilhas')
            );
...
var runLayer = omnivore.kml('" . Url::to(['kml/armadilha', 'except' => $model->isNewRecord ? null : $model->id]) . "')
.on('ready', function() {
...
function coordinatesToInput(coordinates) {
    $('#armadilha-latitude').val(JSON.stringify(coordinates[1]));
    $('#armadilha-longitude').val(JSON.stringify(coordinates[0]));
}
";

if ($model->latitude && $model->longitude) :
    $javascript .= "
        var armadilha = L.marker([" . $model->latitude . ", " . $model->longitude . "]).addTo(featureGroup);
        armadilha.editing.enable();
    ";
endif;
